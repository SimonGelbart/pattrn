...
| Routing preview | Pre-split route matching uses the core segmented matcher after route paths are already split. | Present | Present | `tests/Pattrn.Routing.Tests/RoutingPerformanceSmokeTests.cs`; `tests/Pattrn.Routing.Tests/RoutePatternExtensionTests.cs`; `RoutingBenchmarks.RouteIndex_MatchPreSplitToSpan`; workflow `Routing preview` grouped rows | Covered |  |
...
| Benchmark pipeline | Local grouped-result comparison compares two existing `summaries/grouped-results.json` files from historical artifacts and writes `benchmark-comparison.md` plus `benchmark-comparison.json`. | N/A | N/A | `tools/benchmarks/compare_benchmarks.py`; see [Local grouped-result comparison](#local-grouped-result-comparison). | Covered |  |
| Benchmark pipeline | CI/PR benchmark comparison integration for candidate benchmark runs, including artifact selection, job-summary output, comparison artifact upload/download, and optional PR presentation. Comparisons should remain advisory by default unless a later policy changes that. | N/A | N/A | No CI/PR comparison step is implemented in `.github/workflows/benchmarks.yml`; local comparison remains historical artifact comparison only. | Missing | [#70](https://github.com/SimonGelbart/pattrn/issues/70) |
