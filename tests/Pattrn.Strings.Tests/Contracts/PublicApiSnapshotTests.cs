...
namespace Pattrn.Strings.Tests.Contracts;
...
public sealed class PublicApiSnapshotTests
{
    ...
    public async Task PublicApiMatchesShippedSnapshot()
    {
        ...
        var updateSnapshotPath = Environment.GetEnvironmentVariable("PUBLIC_STRINGS_API_SNAPSHOT_PATH");
        ...
        {
            throw new FileNotFoundException("The strings public API snapshot was not copied to the test output directory.", snapshotPath);
        }
    ...
    {
        var assembly = typeof(PattrnStringExtensions).Assembly;
    ...
    private static string FormatConstructor(ConstructorInfo constructor) => $".ctor({FormatParameters(constructor.GetParameters())})";
    ...
    private static string FormatParameters(ParameterInfo[] parameters) => string.Join(", ", parameters.Select(FormatParameter));
    ...
    private static string FormatParameter(ParameterInfo parameter)
    {
        ...
        var defaultValue = parameter.HasDefaultValue ? $" = {FormatDefaultValue(parameter.DefaultValue, parameterType)}" : string.Empty;
    ...
    private static string FormatTypeName(Type type)
    {
        ...
        if (type.IsGenericType)
        {
            ...
            if (tickIndex >= 0) typeDefinitionName = typeDefinitionName[..tickIndex];
